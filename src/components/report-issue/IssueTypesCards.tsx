...
export const IssueTypesCards = ({
    setSelectedIssueType,
}: IssueTypesCardsProps) => {
    ...
    return (
        <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 flex flex-row gap-4 flex-wrap">
            {onButton(
                { setSelectedIssueType },
                'bug',
                strings['report.issue.type.bug'],
                strings['report.issue.type.bug.desc'],
            )}
            {onButton(
                { setSelectedIssueType },
                'feature',
                strings['report.issue.type.feature'],
                strings['report.issue.type.feature.desc'],
            )}
            {onButton(
                { setSelectedIssueType },
                'question',
                strings['report.issue.type.question'],
                strings['report.issue.type.question.desc'],
            )}
            {onButton(
                { setSelectedIssueType },
                'other',
                strings['report.issue.type.other'],
                strings['report.issue.type.other.desc'],
            )}
        </div>
    )
}
...
export function onButton(
    { setSelectedIssueType }: IssueTypesCardsProps,
    issueType: string,
    title: string,
    description: string,
) {
